...
model_50k = pickle.load(open(
    r'C:\Users\yogen\OneDrive\Desktop\SAVED MODELS\FIRST_AADHAR_50K_ENROLMENT (1).pkl', 'rb'
))
model_100k = pickle.load(open(
    r'C:\Users\yogen\OneDrive\Desktop\SAVED MODELS\AADHAR_100K_ENROLMENT.pkl', 'rb'
))
model_150k = pickle.load(open(
    r'C:\Users\yogen\OneDrive\Desktop\SAVED MODELS\AADHAR_150K_ENROLMENT.pkl', 'rb'
))

scaler = pickle.load(open(
    r'C:\Users\yogen\OneDrive\Desktop\SAVED MODELS\scaler file\scaler.sav', 'rb'
))
...
location_df = pd.read_csv(
    "C:/Users/yogen/Downloads/location_stats.csv"
)

# ======================================
# DEBUG (Hackathon Safe)
# ======================================
...
if st.button("🔍 Predict Enrolment Risk"):
    ...
    if row.empty:
        ...
        if prediction[0] == 1:
            st.error(f"🚨 **SUSPICIOUS ENROLMENT DETECTED**")
            st.markdown(f"<div class='risk'>Risk Probability: {probability:.2f}</div>",
                        unsafe_allow_html=True)
        else:
            st.success(f"✅ **NORMAL ENROLMENT**")
            st.markdown(f"<div class='safe'>Risk Probability: {probability:.2f}</div>",
                        unsafe_allow_html=True)
