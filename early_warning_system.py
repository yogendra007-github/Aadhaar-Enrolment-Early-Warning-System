...
*Hackathon Ready | Explainable AI*
""")

st.markdown("---")

# ======================================
# Load Models (GitHub-safe paths)
# ======================================
BASE_DIR = os.path.dirname(__file__)

@st.cache_resource
def load_models():
    model_50k = pickle.load(open(os.path.join(BASE_DIR, "models/FIRST_AADHAR_50K_ENROLMENT (1).pkl"), "rb"))
    model_100k = pickle.load(open(os.path.join(BASE_DIR, "models/AADHAR_100K_ENROLMENT.pkl"), "rb"))
    model_150k = pickle.load(open(os.path.join(BASE_DIR, "models/AADHAR_150K_ENROLMENT.pkl"), "rb"))
    scaler = pickle.load(open(os.path.join(BASE_DIR, "models/scaler.sav"), "rb"))
...
# Load Location Data
# ======================================
location_df = pd.read_csv(os.path.join(BASE_DIR, "data/location_stats.csv"))
...
with c1:
    st.markdown(f"<div class='metric-box'>👤<br><b>Age</b><br>{age}</div>", unsafe_allow_html=True)

with c2:
    st.markdown(f"<div class='metric-box'>📍<br><b>Pincode</b><br>{pincode}</div>", unsafe_allow_html=True)

with c3:
    st.markdown(f"<div class='metric-box'>🧠<br><b>Model</b><br>{model_choice}</div>", unsafe_allow_html=True)
...
if st.button("🔍 Predict Enrolment Risk"):
    ...
    if row.empty:
        ...
        input_data = np.array([[age, pincode_total, district_avg, state_avg, pincode_total]])
        ...
        if prediction == 1:
            ...
            st.markdown(f"<div class='risk'>Risk Probability: {probability:.2f}</div>", unsafe_allow_html=True)
        else:
            st.success("✅ **NORMAL ENROLMENT**")
            st.markdown(f"<div class='safe'>Risk Probability: {probability:.2f}</div>", unsafe_allow_html=True)
